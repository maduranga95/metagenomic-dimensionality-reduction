...
def train(model, normalize_frequencies, epochs):
    global last_loss
    last_loss = '%.4E' % Decimal(
        model.fit(normalize_frequencies, normalize_frequencies, epochs=epochs).history['loss'][-1])


def auto_train(data_files_info, epochs_list, model_info=None, plot=True, new_model=False, reload=False, peek=False,
               start_epoch=0, use_model="", plot_perc=.1, start_photo_index=0):
    global model
    global camera
    global photo_index
    ...
    for i in epochs_list + ['m', 0]:
        ...
        caption = ("[" + "-".join([str(x) for x in model_info]) + "]          ") + "epoch: " + str(
            epoch_count) + "          loss: " + str(last_loss) if last_loss else ""
...
def pd_vstack(dfs):
    ...
    for i in dfs:
        if big_list is None:
            ...
            big_list = big_list.append(i, ignore_index=True)
    return big_list


def save_model(epochs="", next_photo_index=""): model.save(
    "./models/" + "sigmoid_myco_new"+env['id'] + (("_" + str(epochs).zfill(12)) if epochs else "") + (
        ("_" + str(next_photo_index).zfill(12)) if next_photo_index else ""))


def save_video(id, epochs=""):  call("ffmpeg -framerate 1 -i " + str(id) + "_%12d.png " + str(id) + (
            "_[" + "-".join([str(x) for x in model_info]) + "]_") + str(int(time.time() * 1000)) + "_" + str(
    epochs) + ".mp4", cwd="./figures", shell=True)
...
def plot_files(files, model, perc=.1, caption="", intra_pos=True):
    ...
    for file_index in range(len(files)):
        if (file_index not in _normalized_frequencies_for_file_cache):
            _normalized_frequencies_for_file_cache[file_index] = get_normalized_frequencies_for_file(
                files[file_index][0:int(len(files[file_index]))])
    ...
    sns.scatterplot(ax=axs[0], x='X', y='Y', hue='Sequence', data=df, edgecolor="none", alpha=1,
                    hue_order=get_file_1st_names())
    axs[0].legend(bbox_to_anchor=(1.0, 1), loc=2, borderaxespad=1.)

    flatui = ["#9b59b6", "#3498db", "#95a5a6", "#e74c3c", "#34495e", "#2ecc71"]
    my_cmap = ListedColormap(sns.color_palette(flatui).as_hex())

    sns.scatterplot(ax=axs[1], x='X', y='Y', hue='Mean Position', style='Sequence', style_order=get_file_1st_names(),
                    data=df, edgecolor="none", alpha=1, palette="gist_rainbow_r")
    ...
    # plt.title(caption)
    plt.savefig("figures/" +"sigmoid_myco_new"+ env['id'] + "_" + str(photo_index).zfill(12), bbox_inches='tight')
    photo_index += 1
...
def restore_training(id, data_files_info, epochs_list):
    ...
    auto_train(data_files_info, epochs_list=epochs_list, start_epoch=start_epoch,
               use_model=str(sorted(glob.glob("./models/" + str(id) + "*"))[-1].split("\\")[-1]), new_model=False,
               start_photo_index=photo_index)
...
def build_data_files_info(dir):
    ...
    data_files_info = [dict(file=dir + "/" + files[i], len_mean=len_mean, len_sd=len_sd, count=count) for i in
                       range(len(files))]
...
data_files_info = build_data_files_info("data/mycobacterium")

# epochs = [20,'m']*10+[50]*10+[100]*10+[200,'m']*10+[500,'m']*10+['m']+[1000,'m']*50
epochs = [200, 'm'] * 1 + [500, 'm'] * 2 + [5000, 'm'] * 25
