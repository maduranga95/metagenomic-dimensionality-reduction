...
def build_model(model_info):
    ...
    model = Sequential()
    model.add(Dense(layers_info[1], input_dim=layers_info[0],activation = 'tanh'))
    for i in range(2,len(layers_info)):
        model.add(Dense(layers_info[i], activation = 'tanh'))
...
def save_model(epochs="", next_photo_index=""):model.save("./models/" +"new_4_"+ env['id'] + (("_" + str(epochs).zfill(12)) if epochs else "") + (("_" + str(next_photo_index).zfill(12)) if next_photo_index else ""))
...
def plot_files(files, model, model_info, perc = .1, caption =""):
    ...
    ax.grid(False)
    plt.title(caption)
    plt.savefig("figures/"+"new_4_"+env['id'] +"_"+ str(photo_index).zfill(12), bbox_inches='tight')
    photo_index +=1
    #plt.show()
...
data_files_info = build_data_files_info("data/selected_5")


epochs = [20,'m']*10+[50]*10+[100]*10+[200,'m']*10+[500,'m']*10+['m']+[1000,'m']*200
